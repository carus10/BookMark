// fileHandler_paste_fixed.js - Enhanced file upload and text paste processing module with improved word extraction
...
const FileHandler = {
    ...
    processPastedText: function(title, content) {
        ...
        try {
            // Split content into words - using improved word extraction
            const words = this.extractWordsImproved(content);
    ...
    // Improved word extraction function
    extractWordsImproved: function(text) {
        if (!text || typeof text !== 'string') {
            return [];
        }
        
        // Normalize text - handle different Unicode representations
        const normalizedText = text.normalize('NFC');
        
        // Preserve paragraph structure for better reading
        const paragraphs = normalizedText.split(/\n+/);
        let allWords = [];
        
        paragraphs.forEach(paragraph => {
            // Clean paragraph
            const cleanedParagraph = this.cleanTextPreservingWords(paragraph);
            ...
            const sentences = this.splitIntoSentences(cleanedParagraph);
            
            sentences.forEach(sentence => {
                // Extract words from sentence
                const sentenceWords = this.extractWordsFromSentence(sentence);
                
                // Add sentence words to all words
            ...
            if (paragraphs.indexOf(paragraph) < paragraphs.length - 1) {
                allWords.push('');  // Empty string as paragraph break
            }
        });
        
        // Filter out any remaining empty strings
        return allWords.filter(word => word !== null && word !== undefined);
    },
    
    // Clean text while preserving word integrity
    cleanTextPreservingWords: function(text) {
        if (!text) return '';
        
        // Replace multiple spaces with a single space
        let cleaned = text.replace(/\s+/g, ' ');
        
        // Remove control characters but preserve Unicode letters and symbols
    ...
    splitIntoSentences: function(text) {
        ...
        // This regex handles common sentence endings while avoiding splitting on abbreviations
        return text.split(/(?<=[.!?])\s+|(?<=[.!?])$/);
    },
    
    // Extract words from a sentence
    extractWordsFromSentence: function(sentence) {
        if (!sentence) return [];
        ...
        const words = [];
        const wordMatches = trimmedSentence.match(/[\w\u00C0-\u017F\u0400-\u04FF']+(?:[.,;:!?])?|\S/g) || [];
        
        wordMatches.forEach(word => {
            // Further clean each word
    ...
    cleanWord: function(word) {
        ...
        // Handle common cases where punctuation should be separated
        // For example: "word." becomes ["word", "."]
        const punctuationAtEnd = cleaned.match(/^(.*?)([.,;:!?]+)$/);
        if (punctuationAtEnd) {
            const [, wordPart, punctuation] = punctuationAtEnd;
            
            // If it's a single character followed by punctuation, keep them together
            // This handles abbreviations like "e.g." or "i.e."
            if (wordPart.length <= 1) {
                return cleaned;
            }
            
            // Otherwise, separate the punctuation
            return wordPart;
        }
    ...
    // Legacy extract words function (kept for compatibility)
    extractWords: function(text) {
        return this.extractWordsImproved(text);
    },
    
    // Legacy filter unnecessary content (kept for compatibility)
    filterUnnecessaryContent: function(text) {
        return this.cleanTextPreservingWords(text);
    },
    ...
    processTxtFile: function(file, loadingIndicator) {
        ...
        reader.onload = (event) => {
            try {
                ...
                // Extract words using improved method
                const words = this.extractWordsImproved(text);
    ...
    // Process all PDF pages - using improved word extraction
    processAllPdfPages: async function(pdf, loadingIndicator) {
        console.log("Processing all PDF pages with improved word extraction...");
        let allText = '';
        ...
        for (let i = startPage; i <= endPage; i++) {
            try {
                ...
                if ((i - startPage) % 5 === 0 || i === endPage) {
                    console.log(`Processing PDF: ${i}/${pdf.numPages} pages completed`);
                }
            } catch (pageError) {
        ...
        // Process all text with improved word extraction
        return this.extractWordsImproved(allText);
    },
